...
- [x] @step full-write-refresh-helper-and-tests
...
- [x] 1. On `RefreshExplorationTitle`, ensure the exploration scope has an interaction and
         sessions (same as select) so a never-selected row can still refresh

- [x] 2. When summarizable content exists but `agent_handle` is cold: record a pending
...
- [x] 3. Keep no-op (or hide/disable ↻) when streaming or when `title_refresh_target` is
         `None`; do not wipe existing labels

- [x] 4. Unit or update-level coverage: content present + cold handle eventually applies a
