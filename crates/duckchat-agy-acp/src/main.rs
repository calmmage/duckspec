//! `duckchat-agy-acp` — ACP server on stdio wrapping headless `agy` print mode.
//!
//! Speaks the shared duckchat client dialect (`initialize`, `session/new`,
//! `session/load`, `session/prompt`, cancel). Cold `agy -p` per prompt lands in
//! a later step; this scaffold wires the stdio loop and model advertisement.
