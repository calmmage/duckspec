...
- After an empty change was created, **tight unit**: `/ds-propose` - synthesize
  proposal
- After an empty change was created, **sprawl** (multiple independent outcomes,
  “also / while we’re here”, no single success picture, or multi-user/fork
  concern): rank `/ds-simplicity-audit` - cut scope, then `/ds-propose` -
  synthesize proposal
- Existing change: the stage it is actually ready for
- Durable non-change knowledge: `/ds-codex` - steward durable knowledge

Do not auto-start another stage. Do not force `/ds-simplicity-audit` on every
change — judgment only when sprawl is present.
