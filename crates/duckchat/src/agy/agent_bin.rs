//! Resolve the owned `duckchat-agy-acp` agent binary for AGY turns.
//!
//! Order (first match wins):
//! 1. `DUCKCHAT_AGY_ACP` environment override (selected even if missing —
//!    spawn fails later with a typed error)
//! 2. Sibling of the running executable named `duckchat-agy-acp`
//! 3. `PATH` lookup for `duckchat-agy-acp`
...
/// Environment variable that forces a specific AGY ACP agent binary path.
pub const AGY_ACP_ENV: &str = "DUCKCHAT_AGY_ACP";

/// Binary name of the owned AGY ACP agent.
pub const AGY_ACP_BIN: &str = "duckchat-agy-acp";

/// Resolve the AGY ACP agent binary path.
///
/// Returns [`Error::Spawn`] when no override is set and neither a sibling nor
/// a `PATH` entry is available.
pub fn resolve_agy_acp_binary() -> Result<PathBuf, Error> {
    let env_override = env::var_os(AGY_ACP_ENV).filter(|v| !v.is_empty());
...
/// Build an [`AgentLaunch`] that spawns the resolved AGY ACP agent.
///
...
pub fn agy_acp_launch() -> AgentLaunch {
    AgentLaunch::new(|| {
        let path = resolve_agy_acp_binary().unwrap_or_else(|_| PathBuf::from(AGY_ACP_BIN));
...
///
/// - `env_override`: value of `DUCKCHAT_AGY_ACP` when set
/// - `current_exe`: path of the running executable (for sibling search)
/// - `path_lookup`: `PATH` search for [`AGY_ACP_BIN`]
pub(crate) fn resolve_with(
    ...
    {
        let sibling = dir.join(AGY_ACP_BIN);
        if sibling.is_file() {
            return Ok(sibling);
        }
    }

    if let Some(path) = path_lookup(AGY_ACP_BIN) {
        return Ok(path);
    }

    Err(Error::Spawn(format!(
        "{AGY_ACP_BIN} not found: set {AGY_ACP_ENV}, place the binary next to the running executable, or install it on PATH"
    )))
}
...
mod tests {
    ...
    /// @spec harness/agy Agent binary discovery: An explicit env override selects the agent binary
    #[test]
    fn env_override_selects_agent_binary() {
        let _guard = ENV_LOCK.lock().unwrap();
        let override_path = PathBuf::from("/explicit/path/to/duckchat-agy-acp");
        let resolved = resolve_with(
            Some(override_path.clone()),
            Some(Path::new("/app/duckboard")),
            |_| Some(PathBuf::from("/usr/bin/duckchat-agy-acp")),
        )
        .unwrap();
        assert_eq!(resolved, override_path);
    }

    /// @spec harness/agy Agent binary discovery: When env is unset, a sibling of the running executable is used if present
    #[test]
    fn sibling_of_executable_is_used_when_present() {
        let _guard = ENV_LOCK.lock().unwrap();
        let dir = tempdir().unwrap();
        let exe = dir.path().join("duckboard");
        let sibling = dir.path().join(AGY_ACP_BIN);
    ...
    /// @spec harness/agy Agent binary discovery: A missing agent binary fails the turn with a typed error
    #[tokio::test]
    async fn missing_agent_binary_fails_turn_with_typed_error() {
        let _guard = ENV_LOCK.lock().unwrap();

        // No env override, no sibling, no PATH hit → resolve fails with Spawn.
        let err = resolve_with(None, Some(Path::new("/no/such/duckboard")), |_| None).unwrap_err();
        ...
        let launch = AgentLaunch::new(|| {
            tokio::process::Command::new("/nonexistent/duckchat-agy-acp-missing")
        });
    ...
    fn path_lookup_used_when_no_env_or_sibling() {
        let _guard = ENV_LOCK.lock().unwrap();
        let path_hit = PathBuf::from("/opt/bin/duckchat-agy-acp");
