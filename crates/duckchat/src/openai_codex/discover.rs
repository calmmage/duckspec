...
fn parse_skill_frontmatter(path: &Path) -> SkillFrontmatter {
    ...
    for line in frontmatter.lines() {
        if let Some(v) = line.strip_prefix("name:") {
            ...
            order_key = parse_order_key(v.trim().trim_matches('"'));
        }
    }
    SkillFrontmatter {
        name,
...
/// Accept `digits` or `digits.digit`, returning tenths (`3.1` -> 31).
fn parse_order_key(raw: &str) -> Option<u32> {
    ...
    if let Some((whole, frac)) = raw.split_once('.') {
        ...
        raw.parse::<u32>().ok()?.checked_mul(10)
    }
}
...
mod tests {
    ...
    fn skills_under_agents_skills_are_listed() {
        ...
        assert_eq!(cmds[0].order_key, Some(10));
        assert_eq!(cmds[1].order_key, Some(20));
        assert!(cmds.iter().all(|c| c.kind == SlashCommandKind::Agent));
