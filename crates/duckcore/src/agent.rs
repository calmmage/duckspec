//! UI-neutral agent core around `duckchat`.
//!
//! Provider registry, model catalog, turn driver over `tokio::mpsc`, and
//! `AgentEvent` mapping. UI shells add their own subscription/stream adapters.
...
/// Shared AGY provider so its curated model list follows the catalog path.
fn agy_provider() -> &'static AgyProvider {
    static AGY: OnceLock<AgyProvider> = OnceLock::new();
    AGY.get_or_init(AgyProvider::new)
}
...
impl ModelCatalog {
    ...
    pub fn refresh_registered(&self) {
        ...
        self.apply_harness("agy", agy_provider().list_models());
    }
    ...
    pub fn all(&self) -> Vec<ModelInfo> {
        ...
        // Stable harness order: claude-code, grok, openai-codex, agy,
...
/// and then notifies the UI so it re-reads the catalog.
...
impl Harness {
    pub fn dispatch(harness: &str) -> Self {
        match harness {
            "grok" => Harness::Grok,
...
/// Events routed into a UI update loop. Wraps `duckchat::AgentEvent`
/// plus subscription-lifecycle bookends (`Ready`, `CommandsAvailable`,
/// `ProcessExited`).
#[derive(Debug, Clone)]
pub enum AgentEvent {
    Ready(AgentHandle),
...
/// Drive one agent session for `harness`, forwarding mapped events to `sender`.
pub async fn drive_harness(
    ...
    match Harness::dispatch(&harness) {
        ...
        Harness::Agy => {
            drive_provider(AgyProvider::new(), project_root, sender, oneshot_model).await
        }
        Harness::ClaudeCode => {
            drive_provider(
...
/// `sender`, and emit the `Ready` / `ProcessExited` bookends.
pub async fn drive_provider<P: duckchat::Provider + 'static>(
    provider: P,
    project_root: PathBuf,
    sender: mpsc::Sender<AgentEvent>,
    oneshot_model: Option<String>,
...
mod tests {
    ...
    fn harness_selects_provider() {
        ...
        assert_eq!(Harness::dispatch("agy"), Harness::Agy);
        assert_eq!(AgyProvider::new().id(), "agy");
    ...
    fn unset_global_default_is_seeded_from_the_former_built_in_when_that_model_is_in_the_catalog() {
        ...
        assert!(seed_global_default_if_unset(&mut default_model, &catalog));
