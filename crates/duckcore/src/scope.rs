...
pub struct SessionScope {
    ...
    pub change_facts: Option<ChangeScopeFacts>,
    /// When true (change scope + non-empty `inputs.md` on disk), orientation
    /// names the raw-inputs ledger path for re-grounding.
    pub has_inputs_ledger: bool,
}
...
change.";
    ...
    let inputs = if scope.has_inputs_ledger {
        format!(
            " Raw human inputs: `duckspec/changes/{name}/inputs.md` — re-ground on these \
rather than inventing motivation; do not rewrite that file."
        )
    } else {
        String::new()
    };
...
`duckspec/changes/{name}/`.{inputs} {authority}"
        );
    };
    ...
    format!(
        "Current duckspec scope: change `{name}`. Change artifacts live under \
`duckspec/changes/{name}/`. {progress}{next}{review}{inputs} {authority}"
    )
}
...
mod tests {
    ...
    /// @spec session/scope Inputs ledger pointer: Present inputs.md is named in orientation
    #[test]
    fn present_inputs_md_is_named_in_orientation() {
        let scope = SessionScope {
            kind: ScopeKind::Change,
            scope_key: "raw-user-inputs".into(),
            change_facts: Some(ChangeScopeFacts {
                phase: "designing",
                steps_done: 0,
                step_count: 0,
                active_step_tasks: None,
                next_command: Some("ds-spec".into()),
                current_review: None,
            }),
            has_inputs_ledger: true,
        };
        let text = orientation(&scope);
        assert!(
            text.contains("duckspec/changes/raw-user-inputs/inputs.md"),
            "must name inputs path: {text}"
        );
        assert!(
            text.contains("re-ground") || text.contains("inventing motivation"),
            "must direct re-grounding: {text}"
        );
        assert!(
            text.contains("do not rewrite"),
            "must forbid rewriting the ledger: {text}"
        );
    }

    /// @spec session/scope Inputs ledger pointer: Absent inputs.md yields no inputs pointer
    #[test]
    fn absent_inputs_md_yields_no_inputs_pointer() {
        let scope = SessionScope {
            kind: ScopeKind::Change,
            scope_key: "foo".into(),
            change_facts: Some(ChangeScopeFacts {
        ...
        let text = orientation(&scope);
        assert!(
            !text.contains("inputs.md"),
            "must not invent an inputs pointer: {text}"
        );
        assert!(
            !text.contains("Raw human inputs"),
            "must not invent an inputs pointer: {text}"
        );
    }
}
