...
/// Build pilot mode selected by a system `/build-*` submit.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum BuildPilotMode {
    Auto,
    Fast,
}
...
pub enum SubmitSlash {
    /// Bare system command `/help` — local handler, no agent turn.
    LocalHelp,
    /// System `/build-auto` / `/build-fast` [args] — local classification;
...
/// Duckboard-local system commands (`help`, build pilot).
pub fn system_registry() -> Vec<SlashCommand> {
    vec![
        ...
        SlashCommand {
            name: "build-auto".into(),
            description: "Arm auto build pilot and kick workflow".into(),
            kind: SlashCommandKind::System,
            order_key: None,
        },
        SlashCommand {
            name: "build-fast".into(),
            description: "Arm fast build pilot and kick workflow".into(),
            kind: SlashCommandKind::System,
            order_key: None,
        },
    ]
...
pub fn agent_prompt_for_recovery(user_text: &str) -> Option<String> {
    match parse_submit_slash(user_text) {
        SubmitSlash::LocalHelp | SubmitSlash::LocalBuildPilot { .. } => None,
...
pub fn parse_submit_slash(text: &str) -> SubmitSlash {
    ...
    // `/build-auto` / `/build-fast` with optional free-text args (not bare-only).
    if let Some(rest) = trimmed.strip_prefix('/')
        && !rest.starts_with('/')
        && !rest.is_empty()
    {
        ...
        let args = rest[cmd_end..].trim();
        match cmd {
            "build-auto" => {
                return SubmitSlash::LocalBuildPilot {
                    mode: BuildPilotMode::Auto,
                    args: args.to_string(),
                };
            }
            "build-fast" => {
                return SubmitSlash::LocalBuildPilot {
                    mode: BuildPilotMode::Fast,
                    args: args.to_string(),
                };
            }
            _ => {}
    ...
    // Bare `/help` only (not other system names — those have dedicated routes).
...
mod tests {
    ...
    /// @spec chat/slash-commands Kinded completion catalog: build-auto and build-fast are System
    #[test]
    fn build_auto_and_build_fast_are_system() {
        // GIVEN the system registry includes commands named `build-auto` and `build-fast`
        let system = system_registry();
        // WHEN the completion catalog is built
        let catalog = build_completion_catalog(system, vec![]);
        // THEN both entries are present with kind System
        let auto = catalog
            .iter()
            .find(|c| c.name == "build-auto")
            .expect("build-auto");
        let fast = catalog
            .iter()
            .find(|c| c.name == "build-fast")
            .expect("build-fast");
        assert_eq!(auto.kind, SlashCommandKind::System);
        assert_eq!(fast.kind, SlashCommandKind::System);
    }

    /// @spec chat/slash-commands Build pilot system classification: build pilot names are system registry commands
    #[test]
    fn build_pilot_names_are_system_registry_commands() {
        ...
        assert!(is_system_command_name("build-auto"));
        assert!(is_system_command_name("build-fast"));
    }

    /// @spec chat/slash-commands Build pilot system classification: build-auto with args classifies as local build pilot
    #[test]
    fn build_auto_with_args_classifies_as_local_build_pilot() {
        // GIVEN composer submit text `/build-auto add pilot that auto-sends next`
        // WHEN the submit is classified
        let route = parse_submit_slash("/build-auto add pilot that auto-sends next");
        // THEN local build-pilot auto mode with those arguments
        match route {
            SubmitSlash::LocalBuildPilot { mode, args } => {
                assert_eq!(mode, BuildPilotMode::Auto);
                assert_eq!(args, "add pilot that auto-sends next");
            }
            other => panic!("expected LocalBuildPilot, got {other:?}"),
        }
    }

    /// @spec chat/slash-commands Build pilot system classification: bare build-fast classifies as local build pilot
    #[test]
    fn bare_build_fast_classifies_as_local_build_pilot() {
        // GIVEN composer submit text `/build-fast`
        // WHEN the submit is classified
        let route = parse_submit_slash("/build-fast");
        // THEN local build-pilot fast mode with empty arguments
        match route {
            SubmitSlash::LocalBuildPilot { mode, args } => {
                assert_eq!(mode, BuildPilotMode::Fast);
