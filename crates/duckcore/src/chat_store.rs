...
/// Full title write shared by first-turn auto-title and force refresh: session
/// title (honoring `force`) plus optional exploration `display_name`. Empty or
/// rejected titles leave both unchanged. Returns whether the session title was
/// written.
pub fn apply_title_labels(
    session: &mut ChatSession,
    exploration: Option<&mut Exploration>,
    title: &str,
    force: bool,
) -> bool {
    if !apply_session_title_value(session, title, force) {
        return false;
    }
    if let Some(exp) = exploration
        && let Some(written) = session.title.as_deref()
    {
        exp.display_name = written.to_string();
    }
    true
}
...
fn chats_root(project_root: Option<&Path>) -> PathBuf {
    crate::paths::data_dir(project_root).join("chats")
}
...
///
/// After a successful write, when `project_root` is set, refresh the change
/// package inputs ledger if `session.scope` names an existing change directory
/// (no-op for exploration/caps/codex and for scopes without a change dir).
pub fn save_session(session: &ChatSession, project_root: Option<&Path>) -> anyhow::Result<()> {
    ...
    if let Some(root) = project_root
        && let Err(e) = crate::inputs_ledger::export_inputs_for_change(&session.scope, root)
    {
        tracing::warn!(
            scope = %session.scope,
            "failed to export change inputs ledger: {e}"
        );
    }
    Ok(())
}
...
pub fn delete_project_data(project_root: &Path) {
    let dir = crate::paths::data_dir(Some(project_root));
...
///
/// After migration, exports the inputs ledger for `to` when that name has a
/// change directory (exploration→change promotion path).
pub fn merge_scope(from: &str, to: &str, project_root: Option<&Path>) {
    ...
    if let Some(root) = project_root
        && let Err(e) = crate::inputs_ledger::export_inputs_for_change(to, root)
    {
        tracing::warn!(to, "failed to export inputs ledger after scope merge: {e}");
    }
}
...
/// the idea file when the exploration is linked to an idea (started from one
/// or minted on first tag). Linked explorations still appear on the live
/// CHANGE list; they stay off the archived list when idea-owned. `archived_at`
/// is set when soft-archived in duckboard; absent means live.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl Exploration {
    ...
    /// Change picker / Dashboard Explorations: not archived (idea-linked rows
    /// remain visible so the CHANGE queue can show marks/tags).
...
pub fn load_explorations(project_root: Option<&Path>) -> (Vec<Exploration>, usize) {
    let path = crate::paths::data_dir(project_root).join("explorations.json");
    let Ok(data) = std::fs::read_to_string(&path) else {
        return (vec![], 0);
    };
...
pub fn save_explorations(
    ...
    let dir = crate::paths::data_dir(project_root);
...
mod tests {
    ...
    fn refresh_with_no_content_leaves_labels_unchanged() {
        ...
        // WHEN refresh has no summarizable target, the full-write path is not used.
        assert!(title_refresh_target(&session).is_none());
        assert!(!apply_title_labels(
            &mut session,
            Some(&mut exp),
            "",
            true
    ...
    fn refresh_overwrites_existing_title_and_exploration_label() {
        ...
        // Full-write helper is the same path production force-refresh uses.
        assert!(apply_title_labels(
            &mut session,
            Some(&mut exp),
            "New direction",
        ...
        // Auto first-turn path would not overwrite without force.
        assert!(!apply_title_labels(
            &mut session,
            Some(&mut exp),
            "Should not stick",
    ...
    fn failed_or_empty_refresh_leaves_labels_unchanged() {
        ...
        for result in [
            ...
            if let Some(title) = accept_title_summary_result(&result) {
                apply_title_labels(&mut session, Some(&mut exp), &title, true);
            }
        }
        assert_eq!(exp.display_name, "Keep me");
        assert_eq!(session.title.as_deref(), Some("Keep me"));
    ...
    /// Step 04: cold handle defers refresh; once a title is available the
    /// full-write path still force-applies session + exploration labels.
    #[test]
    fn cold_handle_deferred_refresh_still_uses_full_write() {
        let mut session = session_with(vec![user_msg("rename and retitle work")]);
        session.title = Some("Stale".into());
        assert!(title_refresh_target(&session).is_some());
        // Cold: no handle — production sets pending_title_refresh; when Ready
        // later delivers a summary, apply_title_labels is the write path.
        let mut exp = Exploration {
            id: "exploration-1".into(),
        ...
        assert_eq!(exp.display_name, "Fresh title");
    }
    ...
    fn delete_project_data_idempotent() {
        ...
        with_home(tmp.path(), || {
            ...
            let data = crate::paths::data_dir(Some(&root));
    ...
    // moved to duckboard: in_flight_turn_survives_promotion (needs area::)


    // moved to duckboard: eager_flush_persists_streamed_messages_before_turn_completion (needs area::)
  ...
  "messages": [
    ...
    // moved to duckboard: eager_flush_includes_pending_reasoning_as_reasoning_content (needs area::)
    ...
    fn delete_project_data_removes_chats_and_explorations() {
        ...
        with_home(tmp.path(), || {
            ...
            assert!(!crate::paths::data_dir(Some(&root_a)).exists());
            // Project B is untouched.
            let data_b = crate::paths::data_dir(Some(&root_b));
